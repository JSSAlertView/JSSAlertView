...
public func satisfyAnyOf<T,U>(_ matchers: U...) -> NonNilMatcherFunc<T> where U: Matcher, U.ValueType == T {
    return satisfyAnyOf(matchers)
}

internal func satisfyAnyOf<T,U>(_ matchers: [U]) -> NonNilMatcherFunc<T> where U: Matcher, U.ValueType == T {
