...
internal class AssertionWaitLock: WaitLock {
    fileprivate var currentWaiter: WaitingInfo? = nil
    init() { }
...
internal class AwaitPromise<T> {
    fileprivate(set) internal var asyncResult: AwaitResult<T> = .incomplete
    fileprivate var signal: DispatchSemaphore
...
internal class Awaiter {
    ...
    fileprivate func createTimerSource(_ queue: DispatchQueue) -> DispatchSource {
    ...
    func performBlock<T>(
        _ closure: @escaping ((T) -> Void) throws -> Void) -> AwaitPromiseBuilder<T> {
            let promise = AwaitPromise<T>()
