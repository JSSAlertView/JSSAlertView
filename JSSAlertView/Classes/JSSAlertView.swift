...
open class JSSAlertView: UIViewController {
    ...
    var closeActionWithTextField: ((String?)->Void)!
    var timeLeft: UInt?
    ...
    open override func viewDidLayoutSubviews() {
        ...
        if self.textField != nil {
            yPos += padding / 2
            self.textField.frame = CGRect(x: self.padding, y: yPos, width: self.alertWidth - (self.padding*2), height: 45.0)
            yPos += ceil(self.textField.frame.height)
        }
    ...
    public func show(_ viewController: UIViewController,
        ...
        if textField {
            ...
            self.textField.layer.borderColor = textColor.withAlphaComponent(0.6).cgColor
    ...
    func addActionWithTextHandler(_ completion: @escaping (String?) -> Void) {
        self.closeActionWithTextField = completion
    }
        ...
        UIView.animate(withDuration: 0.3, delay: 0, usingSpringWithDamping: 0.7, initialSpringVelocity: 0.5, options: [], animations: {
            ...
            UIView.animate(withDuration: 0.1, animations: {
                ...
                self.dismiss(animated: false, completion: {
                    ...
                    if withCallback {
                        if let textAction = self.closeActionWithTextField, source == .close {
                            guard let textFieldContent = self.textField.text else {
                                textAction(nil)
                                return
                            }
                            ...
                            textAction(textFieldContent)
                        }
                        else if let action = self.closeAction , source == .close {
                            action()
                        }
